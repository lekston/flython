...
class Discrete(library.blocks.Block):

    def _validate(self):
        """Block validation method run by the simulation manager"""

        print("_validate in {}".format(self.__class__.__name__))

        # When -1 inherit sample time from global settings
        if self.sample_time == -1:
            self.sample_time = self._simulation.step

        if (self.sample_time / self._simulation.step) % 1:
        ...
        self._N = round(self.sample_time / self._simulation.step)
